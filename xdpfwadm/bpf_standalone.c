...
bpf_num_possible_cpus(void)
{
	...
	while (fgets(buff, sizeof (buff), fp)) {
		...
		if (n == 0) {
			fprintf(stderr, "%s(): Failed to retrieve # possible CPUs!\n"
				      , __FUNCTION__);
			exit(1);
		} else if (n == 1) {
			end = start;
...
/* 	TODO: sizeof(bpf_attr) change from kernel to kernel...
	...
	while (multipart) {
		...
		for (nh = (struct nlmsghdr *) buf; NLMSG_OK(nh, len);
			...
			if (nh->nlmsg_pid != nl_pid) {
				fprintf(stderr, "%s(): Wrong nlmsg_pid(%d)!=nl_pid(%d)\n"
					      , __FUNCTION__, nh->nlmsg_pid, nl_pid);
				ret = -1;
				goto done;
			}
			if (nh->nlmsg_seq != seq) {
				fprintf(stderr, "%s(): Wrong nlmsg_seq(%d)!=seq(%d)\n"
			...
			case NLMSG_ERROR:
				...
				if (!err->error)
					continue;
				ret = err->error;
				fprintf(stderr, "%s(): nlmsg error(%s)\n"
...
load_and_attach(const char *event, struct bpf_insn *prog, int size)
{
	...
	if (prog_cnt == MAX_PROGS) {
		fprintf(stderr, "%s(): Too many program loaded (prog_cnt:%d)\n"
	...
	if (fd < 0) {
		fprintf(stderr, "%s(): Error loading BPF program errno=%d (%m)\n%s\n"
	  ...
	  int (*fixup_map) (struct bpf_map_data *, int))
{
	...
	for (i = 0; i < nr_maps; i++) {
		...
		if (map_fd[i] < 0) {
			fprintf(stderr, "%s(): failed to create a map: %d (%m)\n"
				      , __FUNCTION__, errno);
			return -1;
		}
		maps[i].fd = map_fd[i];
		     ...
		     struct bpf_map_data *maps, int nr_maps)
{
	...
	for (i = 0; i < nrels; i++) {
		...
		if (insn[insn_idx].code != (BPF_LD | BPF_IMM | BPF_DW)) {
			fprintf(stderr, "%s(): invalid relo for insn[%d].code 0x%x\n",
		...
		if (match) {
			...
			fprintf(stderr, "%s(): invalid relo for insn[%d] no map_data match\n"
		      ...
		      Elf * elf, Elf_Data * symbols, int strtabidx)
{
	...
	if (!scn || !data_maps) {
		fprintf(stderr, "%s(): Failed to get Elf_Data from maps section %d\n"
	...
	for (i = 0; i < nr_maps; i++) {
		...
		def =
		    (struct bpf_load_map_def *) ((uint8_t *) data_maps->d_buf +
						 offset);
...
bpf_load_from_file(const char *path, int (*fixup_map) (struct bpf_map_data *, int))
{
	...
	for (i = 1; i < ehdr.e_shnum; i++) {
		...
		if (strcmp(shname, "license") == 0) {
			...
			if (data->d_size != sizeof (int)) {
				fprintf(stderr, "%s(): invalid size of version section %zd\n"
	...
	if (!symbols) {
		fprintf(stderr, "%s(): missing SHT_SYMTAB section\n"
			      , __FUNCTION__);
		goto done;
	}
	...
	if (data_maps) {
		...
		if (nr_maps < 0) {
			fprintf(stderr, "%s(): Error: Failed loading ELF maps (errno:%d):%s\n"
